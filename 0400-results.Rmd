...
A summary of habitat modelling outputs is presented in Table \@ref(tab:tab-culvert-bcfp-197534) and a map of the watershed is
...
Electrofishing was conducted at eight sites with results summarised in Tables \@ref(tab:tab-ef-site-weigert) - \@ref(tab:tab-fish-dens-197534) and Figure \@ref(fig:plot-fish-box-197534). Habitat details are summarised in Table \@ref(tab:tab-habitat-summary-197534). 
...
hab_site %>% 
  ...
  filter(str_detect(alias_local_name, "Weigert")) %>%
...
Fish habitat was assessed at four sites within three reaches of Weigert Creek (Tables \@ref(tab:tab_fhap_site) - \@ref(tab:tab_fhap_hu_sum)).
...
Habitat characteristics were gathered at two sites where electrofishing was conducted in 2022 (Figure \@ref(fig:survey-site-map)). Habitat details are summarised in Table \@ref(tab:tab-habitat-summary-197559).
...
The second site electrofished was located approximately 1km upstream of PSCIS crossing 197536. This section was surveyed for `r fpr::fpr_my_priority_info(loc = 'us_ef2', col_pull = 'length_surveyed')`m to the location of a waterfall shown in `r if(gitbook_on){knitr::asis_output("Figure \\@ref(fig:photo-197559-04).")}else(knitr::asis_output("Figure \\@ref(fig:photo-197559-u04)."))` This waterfall was approximately 7m in height, and blocks the upstream migration of all fish. This obstacle is listed in the BC Provincial Obstacles to Fish Passage catalogue [@ministryofwaterlandandresourcestewardship2023ProvincialObstacles]. At the time of survey, the average channel width was 7.5m, the average wetted width was 4.9m, and the average gradient was 2.1%. The dominant substrate was cobbles with boulders sub dominant. The habitat quality was considered moderate with abundant gravels suitable for resident and fluvial westslope cutthroat and bull trout spawning but few pools present that would be suitable for overwintering. 
...
The stream was electrofished at two sites upstream of crossing `r as.character(my_site)` in 2022 with no fish captured at either site (Tables \@ref(tab:ef-site-brule) - \@ref(tab:tab-fish-dens-197559)).
...
hab_site %>% 
  ...
  # filter out two sites that weren't electrofished
  filter(str_detect(alias_local_name, "ef", negate = FALSE)) %>%
  filter(str_detect(alias_local_name, "Brule")) %>%
  arrange(alias_local_name) %>% 
  fpr::fpr_kable()
```
